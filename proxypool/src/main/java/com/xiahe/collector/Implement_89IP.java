...
public class Implement_89IP extends CollectImplement {
    ...
    public void collect() {
        ...
        for (int i = 1; i < 200; i++) {
            try {
                String url = "http://www.89ip.cn/index_" + i + ".html";// 收集网址
    ...
    public List<JSONObject> pattern(String content) {
        ...
        Pattern pattern = Pattern.compile("<td>(\\d{1,3}\\.){3}\\d{1,3}</td><td>\\d+</td>".replaceAll("\\s", ""));
        Matcher matcher = pattern.matcher(content.replaceAll("\\s", ""));
        while (matcher.find()) {
            String[] strings = matcher.group().split("</td><td>".replaceAll("\\s", ""));
            if (strings != null && strings[0] != "") {
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("from", Implement_89IP.class.getSimpleName());
                jsonObject.put("ip", strings[0].replaceAll("<td>", ""));
